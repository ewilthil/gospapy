...
def calculate_gospa(targets, tracks, c, p, alpha=2,
    ...
    dim_targets = len(targets)
    dim_tracks = len(tracks)
    miss_cost = c**p/alpha
    if dim_targets == 0: # All the tracks are false tracks
        gospa_false = miss_cost*dim_tracks
        assignment = dict()
        return gospa_false**(1/p), assignment, 0, 0, gospa_false
    elif dim_tracks == 0: # All the targets are missed
        gospa_missed = miss_cost*dim_targets
        assignment = dict()
        return gospa_missed**(1/p), assignment, 0, gospa_missed, 0
    else: # There are elements in both sets. Compute cost matrix
        cost_matrix = np.zeros((dim_targets, dim_tracks))
        for n_target in range(dim_targets):
            for n_track in range(dim_tracks):
        ...
        num_missed = dim_targets - num_assignments
        num_false = dim_tracks - num_assignments
